...
/**
 * Compute a numeric score for ranking purposes (higher = better).
 *   Mexico (2+1)  → 1000
 *   Doubles dd    → 900 + d   (66=966, 55=955, … 11=911)
 *   Regular       → tens*10 + ones  (e.g. [6,4] → 64)
 */
function scoreRoll(a: number, b: number): number {
...
function rollLabel(a: number, b: number): string {
  const hi = Math.max(a, b);
  const lo = Math.min(a, b);
...
function Mexico({ players }: { players: Player[] }) {
  ...
  const computeLosers = useCallback((res: RollResult[]): Player[] => {
    const minScore = Math.min(...res.map((r) => r.score));
    const loserIds = res.filter((r) => r.score === minScore).map((r) => r.playerId);
    return players.filter((p) => loserIds.includes(p.id));
  }, [players]);
